...
void scan_string(char* buffer, int max_size, char end_btn, uint32_t x, uint32_t y, uint32_t fg_color, uint32_t bg_color) {
    int i = 0;
    while (1) {
        char c = get_key();
        ...
        if (c == '\b') {
            if (i > 0) {
                i--;
                print_char('\b', x + i * FONT_CHAR_WIDTH, y, fg_color, bg_color);
            }
        } else if (c >= ' ' && i < max_size - 1) {
            buffer[i] = c;
            print_char(c, x + i * FONT_CHAR_WIDTH, y, fg_color, bg_color);
            i++;
        }
    }
    buffer[i] = '\0';
    print_char('\n', x + i * FONT_CHAR_WIDTH, y, fg_color, bg_color);
}